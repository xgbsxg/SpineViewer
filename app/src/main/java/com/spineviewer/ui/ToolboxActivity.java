        ...
        protected String doInBackground(Object... params) {
            ...
            try {
                ...
                for (String line : lines) {
                    ...
                    if (inPage) {
                        // 检测 bounds 行：格式为 "  x, y, width, height" 或 "  xy: x, y, width, height"
                        if (trimmed.startsWith("xy:") || trimmed.matches("^\\s*\\d+,\\s*\\d+,\\s*\\d+,\\s*\\d+\\s*$")) {
                            inBounds = true;
                            String[] parts;
                            if (trimmed.startsWith("xy:")) {
                                parts = trimmed.replace("xy:", "").trim().split("\\s*,\\s*");
                            } else {
                            ...
                            if (parts.length >= 4) {
                                try {
                                    ...
                                    String newLine = String.format("  %d, %d, %d, %d", newX, newY, newWidth, newHeight);
